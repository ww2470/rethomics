...
make_annotation_df <- function(annot_result_dir,behaviour_map,annot_levels){
	...
	setnames(ref, colnames(ref),c("region_id","t","user"))
	ref[,region_id:=as.numeric(region_id)]
	ref[,t:=as.numeric(t)]
...
setkeyv(pos_at_t,c('region_id','t'))
...
pl <- ggplot(pdt,aes(ar_diff,phi_diff,shape=behaviour,colour=behaviour)) +
...
data <- dt[region_id==2, list(colour=asleep,t=t/3600,x=x)]
data <- data[t>5 &  t<10]
